...
import recordLife.dao.IAdminDao;
...
import recordLife.pojo.PointAction;
import recordLife.pojo.PointRecord;
	...
	public Map<String, Object> conchIndex(long uId,String operation ,PageBean page) throws UserServiceException {
		...
		try {
			if("NEWS".equals(operation)){
				list = userDao.findConchByNews(page.getPage(),page.getMaxPage());
	...
	public Map<String, Object> seachWeiboAndUser(String key ,int page,String ac) throws UserServiceException {
		...
		try {
			...
			if("weibo".equals(ac)){
				...
				bean.setMaxPage(5);
				//模糊搜索对应关键字的微博
			    List<Weibo> weiboList = weiboDao.findWeiboBySeach(key, bean.getPage(), bean.getMaxPage());
				...
				bean.setMaxPage(5);
				//模糊搜索对应关键字的用户
				List<User> userList = weiboDao.findUserBySeach(key, bean.getPage(), bean.getMaxPage());
