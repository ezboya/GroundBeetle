package recordLife.service.impl;
...
import recordLife.Exception.DataAccessException;
import recordLife.Exception.WeiboServiceException;
import recordLife.Utils.CommPage;
import recordLife.Utils.Constant;
import recordLife.Utils.PageBean;
import recordLife.Utils.TimeFormat;
import recordLife.dao.IFriendDao;
import recordLife.dao.IUserDao;
import recordLife.dao.IWeiboDao;
import recordLife.pojo.User;
import recordLife.pojo.Weibo;
import recordLife.pojo.WeiboCollect;
import recordLife.pojo.WeiboComm;
import recordLife.pojo.WeiboLike;
import recordLife.pojo.WeiboTrans;
...
public class IWeiboServiceImpl implements IWeiboService{
	...
	public List<WeiboComm> findWeiboComm(long wId,CommPage commPage) throws WeiboServiceException {
		...
		User user = null;
		try {
			list = dao.findWeiboComm(wId,commPage.getPage(),commPage.getMaxPage());
			for(int i=0;i<list.size();i++){
	...
	public List<Weibo> findHomeWeibo(int tag, PageBean page,String weiboTime) throws WeiboServiceException {
		List<Weibo> list = null;
		try {
			System.out.println(0+"--------"+page.getMinPage()+"-----"+page.getMaxPage()+"--------"+weiboTime);
			list = dao.findWeiboOfHome(0, page.getPage(), page.getMaxPage(), weiboTime);
		} catch (DataAccessException e) {
			e.printStackTrace();
		}
		return list;
