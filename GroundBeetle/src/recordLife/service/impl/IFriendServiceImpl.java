...
import recordLife.Exception.FriendServiceException;
...
import recordLife.pojo.Friend;
import recordLife.pojo.Message;
import recordLife.pojo.User;
import recordLife.pojo.Weibo;
import recordLife.service.IFriendService;
...
public class IFriendServiceImpl implements IFriendService{
	...
	public List<Weibo> findFriendWeibo(long uId,PageBean page) throws FriendServiceException {
		...
		try {
			list = dao.findFriendWeibo(uId, page.getPage(), page.getMaxPage());
	...
	public List<User> findAttUser(long uId,PageBean page) throws FriendServiceException {
		...
		try {
			attUser = dao.findAttUser(uId, page.getPage(), page.getMaxPage());
		} catch (DataAccessException e) {
			e.printStackTrace();
		}
		return attUser;
	...
	public List<User> findFansUser(long uId,PageBean page) throws FriendServiceException {
		List<User> list = null;
		try {
			list = dao.findFansUser(uId, page.getPage(),page.getMaxPage());
