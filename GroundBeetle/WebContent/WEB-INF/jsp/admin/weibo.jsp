				...
				<table border="0" align="center" cellpadding="0" cellspacing="0"
					...
					<tr>
						...
						<td align="right" colspan="4">
							<form id="form1" name="form1" method="post"
								action="weiboManage?page=1">
								<input name="key" type="text" id="textfield"
									placeholder="输入微博内容。。。" /> <input name="button" type="submit"
					...
					<c:forEach items="${weiboList}" var="weibo">
						<tr>
							...
							<td><c:if test="${weibo.state == 0}">正常</c:if> <c:if
									test="${weibo.state == 1}">被删除</c:if></td>
							<td><c:if test="${weibo.state == 0}">
									<input onclick="scwb(${weibo.id})" type="button"
										class="btnsearch" id="button" value="删除" />
								</c:if> <c:if test="${weibo.state == 1}">
									<input onclick="hfwb(${weibo.id})" type="button"
