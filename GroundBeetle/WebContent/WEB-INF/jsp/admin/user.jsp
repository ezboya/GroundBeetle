				...
				<table border="0" align="center" cellpadding="0" cellspacing="0"
					...
					<tr>
						...
						<td align="right" colspan="4">
							<form id="form1" name="form1" method="post"
								action="userManage?page=1">
								<input name="key" type="text" id="textfield"
									placeholder="输入用户昵称。。。" /> <input name="button" type="submit"
					...
					<c:forEach items="${userList}" var="user">
						<tr>
							...
							<td><c:if test="${user.state == 0}">正常</c:if> <c:if
									test="${user.state == 1}">${user.banTime}解封</c:if></td>
							<td><c:if test="${user.state == 0}">
									<input onclick="fj(${user.id})" type="button" class="btnsearch"
										id="button" value="封禁" />
								</c:if> <c:if test="${user.state == 1}">
									<input onclick="jf(${user.id})" type="button" class="btnsearch"
										id="button" value="解封" />
								</c:if></td>
						</tr>
						<tr>
				...
				<table align="center" id="page">
					<tr>
						<td align="right"><c:if test="${pageBean.page!=1}">
								<a href="userManage?page=${pageBean.page-1}"
