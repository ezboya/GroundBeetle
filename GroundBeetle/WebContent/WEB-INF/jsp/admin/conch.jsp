				...
				<table border="0" align="center" cellpadding="0" cellspacing="0"
					...
					<tr>
						...
						<td align="right" colspan="4">
							<form id="form1" name="form1" method="post"
								action="conchManage?page=1">
								<input name="key" type="text" id="textfield"
									placeholder="输入海螺内容。。。" /> <input name="button" type="submit"
					...
					<c:forEach items="${conchList}" var="conch">
						<tr>
							...
							<td><c:if test="${conch.state == 0}">未解决</c:if> <c:if
									test="${conch.state == 1}">已解决</c:if> <c:if
									test="${conch.state == 2}">已删除</c:if> <c:if
									test="${conch.state == 3}">已删除</c:if></td>
							<td><c:if test="${conch.state == 0}">
									<input onclick="sc(${conch.id})" type="button"
										class="btnsearch" id="button" value="删除" />
								</c:if> <c:if test="${conch.state == 1}">
									<input onclick="sc(${conch.id})" type="button"
										class="btnsearch" id="button" value="删除" />
								</c:if> <c:if test="${conch.state == 2}">
									<input onclick="hf(${conch.id})" type="button"
										class="btnsearch" id="button" value="恢复" />
								</c:if> <c:if test="${conch.state == 3}">
									<input onclick="hf(${conch.id})" type="button"
										class="btnsearch" id="button" value="恢复" />
								</c:if></td>
						</tr>
				...
				<table align="center" id="page">
					<tr>
						<td align="right"><c:if test="${pageBean.page!=1}">
								<a href="weiboManage?page=${pageBean.page-1}"
